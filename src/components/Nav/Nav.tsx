...
const Nav = () => {
  ...
  const OnClickViewOnTop = (id: any) => {
    const element = document.getElementById(id)?.scrollIntoView({ behavior: "smooth" });
    const idParts = id.split('-');
    const targetId = idParts[0];
    setIds(targetId)
    if(element) {
      width < 991 ? element.style.paddingTop = '170px' : element.style.paddingTop = '270px';
    }
  };
